...
interface GlowAreaProps extends HTMLAttributes<HTMLDivElement> {
    size?: number;
}
    ...
    return (
        <div ref={element} className={cn(className, "relative")}>
            ...
            transparent 100%
                ...
                className={cn(
                    className,
                    "absolute pointer-events-none inset-0 dark:mix-blend-lighten mix-blend-multiply after:content-[''] after:absolute after:bg-background/90 after:inset-[.5px] after:rounded-[inherit]",
                )}
            ></div>
            {children}
        </div>
